...
module angle_plate(angle = 90, // To turn the overhang so it's pointing up, use negative degrees, like -90.
                   base_stud_length = 2, base_stud_width = 2, overhang_stud_length = 2, overhang_stud_width = 2)
{
    ...
    if (base_stud_length > base_stud_width)
    {
        ...
        rotate([ 0, 0, 90 ])
            block(height = .33333333333, type = "brick", length = base_stud_length, width = base_stud_width);
    }
    else
    {
        block(height = .33333333333, type = "brick", length = base_stud_length, width = base_stud_width);
    ...
    if (angle >= 0)
    {
        ...
        difference()
        {
            translate([ base_width / 2 + (overhang_height * sin(angle)) + wall_play, 0, base_height ])
                rotate([ 0, angle, 0 ]) translate([ overhang_width / 2, 0, -overhang_height ]) union()
            {
                if (overhang_stud_length > overhang_stud_width)
                {
                    // Lego.scad always makes the longest number the length. Rotate it manually if we want it wider than
                    // long.
                    rotate([ 0, 0, 90 ]) block(height = .33333333333, type = "brick", length = overhang_stud_length,
                                               width = overhang_stud_width, stud_type = "hollow");
                }
                else
                {
                    block(height = .33333333333, type = "brick", length = overhang_stud_length,
                          width = overhang_stud_width, stud_type = "hollow");
                }

                // Fill in the underside of the angled portion. type=baseplate would do this, but it would also make it
                // thinner and give it rounded edges.
                translate([ 0, 0, overhang_height / 2 ])
                    cube([ overhang_width, overhang_length, overhang_height ], true);
            }
            ...
            translate([ 0, 0, base_height + overhang_width / 2 ])
            {
                cube(
                    [
                        base_width + (overhang_height * 2 * sin(angle)) + 0.01,
                        max(base_length, overhang_length) + 0.01,
                        overhang_width
                    ],
                    true);
            }
    ...
    else
    {
        difference()
        {
            translate([ base_width / 2 + wall_play, 0, 0 ]) rotate([ 180, -angle, 180 ])
                translate([ -overhang_width / 2, 0, -base_height + overhang_height ]) union()
            {
                if (overhang_stud_length > overhang_stud_width)
                {
                    // Lego.scad always makes the longest number the length. Rotate it manually if we want it wider than
        ...
        if (angle >= -90)
        {
            ...
            translate([ base_width / 2, base_length / 2, 0 ]) rotate([ 90, 0, 0 ]) linear_extrude(height = base_length)
                polygon(points = [
                    [ 0, 0 ],
                    [ 0, base_height ],
